...
public:
    ReservedRegister(void *drcontext, instrlist_t *ilist, instr_t *where);

    ~ReservedRegister();

    ReservedRegister(const ReservedRegister &) = delete;
...
public:
    DecodedInstruction(void *drcontext, app_pc pc);

    ~DecodedInstruction();

    DecodedInstruction(const DecodedInstruction &);
